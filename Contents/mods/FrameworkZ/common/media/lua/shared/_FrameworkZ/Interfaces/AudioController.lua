...
function FrameworkZ.UI.AudioController:update()
    ...
    if self.AutoLoop and not self.SuppressAutoLoop and self.TrackName and not self:isPlaying() then
        self:playTrack(self.TrackName)
    end
end
...
function FrameworkZ.UI.AudioController:playTrack(audioName)
    ...
    self.SuppressAutoLoop = false
    self.TrackName = audioName
...
function FrameworkZ.UI.AudioController:setLooping(enabled)
    self.AutoLoop = enabled and true or false
