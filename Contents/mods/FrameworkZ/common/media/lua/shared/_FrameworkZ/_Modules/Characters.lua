...
function CHARACTER:Save(callback)
    local isoPlayer = self:GetIsoPlayer() if not isoPlayer then 
        if callback then callback(false, "Missing Iso Player: CHARACTER.Save (callback)") end
        return false, "Missing Iso Player: CHARACTER.Save (return)" 
    end
...
-- B42: getStats() now uses CharacterStat enum — old named get/setX() methods are removed.
-- Helper to read/write a stat with B41 fallback.
local function statsGet(stats, b42enum, b41method) if CharacterStat then return stats:get(b42enum) else return stats[b41method](stats) end end
local function statsSet(stats, b42enum, b41method, value) if CharacterStat then stats:set(b42enum, value) else stats[b41method](stats, value) end end
...
function CHARACTER:GetHunger()
    ...
    if isoPlayer then return statsGet(isoPlayer:getStats(), CharacterStat and CharacterStat.HUNGER, "getHunger") end
    return self.Hunger
end
...
function CHARACTER:SetHunger(hunger)
    self.Hunger = hunger
    local isoPlayer = self:GetIsoPlayer()
    if isoPlayer then statsSet(isoPlayer:getStats(), CharacterStat and CharacterStat.HUNGER, "setHunger", hunger) end
end
...
function CHARACTER:GetThirst()
    local isoPlayer = self:GetIsoPlayer()
    if isoPlayer then return statsGet(isoPlayer:getStats(), CharacterStat and CharacterStat.THIRST, "getThirst") end
    return self.Thirst
end
...
function CHARACTER:SetThirst(thirst)
    self.Thirst = thirst
    local isoPlayer = self:GetIsoPlayer()
    if isoPlayer then statsSet(isoPlayer:getStats(), CharacterStat and CharacterStat.THIRST, "setThirst", thirst) end
end
...
function CHARACTER:GetFatigue()
    local isoPlayer = self:GetIsoPlayer()
    if isoPlayer then return statsGet(isoPlayer:getStats(), CharacterStat and CharacterStat.FATIGUE, "getFatigue") end
    return self.Fatigue
end
...
function CHARACTER:SetFatigue(fatigue)
    ...
    local isoPlayer = self:GetIsoPlayer()
    if isoPlayer then statsSet(isoPlayer:getStats(), CharacterStat and CharacterStat.FATIGUE, "setFatigue", fatigue) end
end
...
function CHARACTER:GetStress()
    local isoPlayer = self:GetIsoPlayer()
    if isoPlayer then return statsGet(isoPlayer:getStats(), CharacterStat and CharacterStat.STRESS, "getStress") end
    return self.Stress
end
...
function CHARACTER:SetStress(stress)
    self.Stress = stress
    local isoPlayer = self:GetIsoPlayer()
    if isoPlayer then statsSet(isoPlayer:getStats(), CharacterStat and CharacterStat.STRESS, "setStress", stress) end
end
...
function CHARACTER:GetPain()
    local isoPlayer = self:GetIsoPlayer()
    if isoPlayer then return statsGet(isoPlayer:getStats(), CharacterStat and CharacterStat.PAIN, "getPain") end
    return self.Pain
end
...
function CHARACTER:SetPain(pain)
    self.Pain = pain
    local isoPlayer = self:GetIsoPlayer()
    if isoPlayer then statsSet(isoPlayer:getStats(), CharacterStat and CharacterStat.PAIN, "setPain", pain) end
end
...
function CHARACTER:GetPanic()
    local isoPlayer = self:GetIsoPlayer()
    if isoPlayer then return statsGet(isoPlayer:getStats(), CharacterStat and CharacterStat.PANIC, "getPanic") end
    return self.Panic
end
...
function CHARACTER:SetPanic(panic)
    self.Panic = panic
    local isoPlayer = self:GetIsoPlayer()
    if isoPlayer then statsSet(isoPlayer:getStats(), CharacterStat and CharacterStat.PANIC, "setPanic", panic) end
end
...
function CHARACTER:GetBoredom()
    local isoPlayer = self:GetIsoPlayer()
    if isoPlayer then return statsGet(isoPlayer:getStats(), CharacterStat and CharacterStat.BOREDOM, "getBoredom") end
    return self.Boredom
end
...
function CHARACTER:SetBoredom(boredom)
    ...
    local isoPlayer = self:GetIsoPlayer()
    if isoPlayer then statsSet(isoPlayer:getStats(), CharacterStat and CharacterStat.BOREDOM, "setBoredom", boredom) end
end
...
function CHARACTER:GetDrunkenness()
    local isoPlayer = self:GetIsoPlayer()
    -- B42: renamed to INTOXICATION
    if isoPlayer then return statsGet(isoPlayer:getStats(), CharacterStat and CharacterStat.INTOXICATION, "getDrunkenness") end
...
function CHARACTER:SetDrunkenness(drunkenness)
    ...
    local isoPlayer = self:GetIsoPlayer()
    -- B42: renamed to INTOXICATION
    if isoPlayer then statsSet(isoPlayer:getStats(), CharacterStat and CharacterStat.INTOXICATION, "setDrunkenness", drunkenness) end
end
...
function CHARACTER:GetEndurance()
    local isoPlayer = self:GetIsoPlayer()
    if isoPlayer then return statsGet(isoPlayer:getStats(), CharacterStat and CharacterStat.ENDURANCE, "getEndurance") end
...
function CHARACTER:SetEndurance(endurance)
    ...
    local isoPlayer = self:GetIsoPlayer()
    if isoPlayer then statsSet(isoPlayer:getStats(), CharacterStat and CharacterStat.ENDURANCE, "setEndurance", endurance) end
end
...
function CHARACTER:GetTemperature()
    local isoPlayer = self:GetIsoPlayer()
    -- B42: moved from getBodyDamage():getTemperature() to CharacterStat.TEMPERATURE
    if isoPlayer then
        if CharacterStat then return isoPlayer:getStats():get(CharacterStat.TEMPERATURE)
        else return isoPlayer:getBodyDamage():getTemperature() end
    end
...
function CHARACTER:SetTemperature(temperature)
    ...
    local isoPlayer = self:GetIsoPlayer()
    -- B42: moved from getBodyDamage():setTemperature() to CharacterStat.TEMPERATURE
    if isoPlayer then
        if CharacterStat then isoPlayer:getStats():set(CharacterStat.TEMPERATURE, temperature)
        else isoPlayer:getBodyDamage():setTemperature(temperature) end
    end
end
...
function CHARACTER:GetWetness()
    local isoPlayer = self:GetIsoPlayer()
    -- B42: moved from getBodyDamage():getWetness() to CharacterStat.WETNESS
    if isoPlayer then
        if CharacterStat then return isoPlayer:getStats():get(CharacterStat.WETNESS)
        else return isoPlayer:getBodyDamage():getWetness() end
    end
    return self.Wetness
end
...
function CHARACTER:SetWetness(wetness)
    ...
    local isoPlayer = self:GetIsoPlayer()
    -- B42: moved from getBodyDamage():setWetness() to CharacterStat.WETNESS
    if isoPlayer then
        if CharacterStat then isoPlayer:getStats():set(CharacterStat.WETNESS, wetness)
        else isoPlayer:getBodyDamage():setWetness(wetness) end
    end
end
...
function CHARACTER:GetSickness()
    local isoPlayer = self:GetIsoPlayer()
    -- B42: moved from getBodyDamage():getFoodSicknessLevel() to CharacterStat.FOOD_SICKNESS
    if isoPlayer then
        if CharacterStat then return isoPlayer:getStats():get(CharacterStat.FOOD_SICKNESS)
        else return isoPlayer:getBodyDamage():getFoodSicknessLevel() end
    end
...
function CHARACTER:SetSickness(sickness)
    ...
    local isoPlayer = self:GetIsoPlayer()
    -- B42: moved from getBodyDamage():setFoodSicknessLevel() to CharacterStat.FOOD_SICKNESS
    if isoPlayer then
        if CharacterStat then isoPlayer:getStats():set(CharacterStat.FOOD_SICKNESS, sickness)
        else isoPlayer:getBodyDamage():setFoodSicknessLevel(sickness) end
    end
end
...
function CHARACTER:GetStats()
    ...
    if isoPlayer then
        ...
        -- B42: named getX() methods removed; use CharacterStat enum with get()
        self.Hunger      = statsGet(stats, CharacterStat and CharacterStat.HUNGER,       "getHunger")
        self.Thirst      = statsGet(stats, CharacterStat and CharacterStat.THIRST,       "getThirst")
        self.Fatigue     = statsGet(stats, CharacterStat and CharacterStat.FATIGUE,      "getFatigue")
        self.Stress      = statsGet(stats, CharacterStat and CharacterStat.STRESS,       "getStress")
        self.Pain        = statsGet(stats, CharacterStat and CharacterStat.PAIN,         "getPain")
        self.Panic       = statsGet(stats, CharacterStat and CharacterStat.PANIC,        "getPanic")
        self.Boredom     = statsGet(stats, CharacterStat and CharacterStat.BOREDOM,      "getBoredom")
        self.Drunkenness = statsGet(stats, CharacterStat and CharacterStat.INTOXICATION, "getDrunkenness")
        self.Endurance   = statsGet(stats, CharacterStat and CharacterStat.ENDURANCE,    "getEndurance")
    end
    return {
...
function CHARACTER:GetAllHealth()
    ...
    if isoPlayer then
        ...
        local function callAny(target, names, default)
            for i = 1, #names do
                local fn = target[names[i]]
        ...
        self.Health = callAny(bodyDamage, { "getOverallBodyHealth" }, self.Health)
        self.Temperature = callAny(bodyDamage, { "getTemperature" }, self.Temperature)
        self.Wetness = callAny(bodyDamage, { "getWetness" }, self.Wetness)
        self.Sickness = callAny(bodyDamage, { "getFoodSicknessLevel", "getSicknessLevel" }, self.Sickness)
        self.ColdStrength = callAny(bodyDamage, { "getColdStrength" }, self.ColdStrength)
        self.HasCold = callAny(bodyDamage, { "getHasACold", "isHasACold", "HasACold" }, self.HasCold)
        
        -- Sync body parts data using proper iteration method
        local bodyPartsData = {}
        local bodyParts = callAny(bodyDamage, { "getBodyParts" }, nil)
        if bodyParts and type(bodyParts.size) == "function" and type(bodyParts.get) == "function" then
...
function CHARACTER:GetTraits()
    ...
    if isoPlayer then
        ...
        if CharacterTrait then
            -- B42: getTraits() removed; use getCharacterTraits():getKnownTraits()
            local knownTraits = isoPlayer:getCharacterTraits():getKnownTraits()
            for i = 0, knownTraits:size() - 1 do
                table.insert(traitsData, knownTraits:get(i):getName())
            end
        else
...
--! Each step is isolated so a failure in one (e.g. an item's OnInstanced hook throwing during
--! equipment/inventory repair) cannot silently skip the remaining steps, such as position/inventory
--! being left unsynced right before a disconnect-triggered save.
...
--! \brief Give a character a defined number of items.
...
function CHARACTER:Restore(characterData)
    ...
    local x = useSpawnPosition and FrameworkZ.Config:GetOption("SpawnX") or savedX
    local y = useSpawnPosition and FrameworkZ.Config:GetOption("SpawnY") or savedY
    ...
    if not resolvedUID then
        if player.GenerateUID then
            resolvedUID = player:GenerateUID()
        else
            resolvedUID = tostring(username) .. "_" .. tostring(characterID or FrameworkZ.Utilities:GetRandomNumber(1, 999999, true))
        end

        if resolvedUID then
            self:SetUID(resolvedUID)
            characterData[FZ_ENUM_CHARACTER_META_UID] = resolvedUID

            local charactersMap = player:GetCharacters()
            if type(charactersMap) == "table" and characterID and type(charactersMap[characterID]) == "table" then
                charactersMap[characterID][FZ_ENUM_CHARACTER_META_UID] = resolvedUID
            end
    ...
    if not self:GetUID() then
        ...
        return false, "Failed to resolve character UID during restore."
    end
...
function CHARACTER:RestoreModel(characterData, reset)
    ...
    local function normalizeColor(colorValue, fallback)
    ...
    -- Set hair (normalize bald/none aliases to empty style ID used by the game)
    local savedHairStyle = characterData[FZ_ENUM_CHARACTER_INFO_HAIR_STYLE]
    if savedHairStyle == nil or savedHairStyle == "" or savedHairStyle == "Bald" or savedHairStyle == "None" then
        humanVisual:setHairModel("")
    else
        humanVisual:setHairModel(savedHairStyle)
    end

    -- Set hair color
    local hairColor = normalizeColor(
        characterData[FZ_ENUM_CHARACTER_INFO_HAIR_COLOR],
        { r = 0.3, g = 0.2, b = 0.2 }
    )
    ...
    if beardStyle == "" or beardStyle == "None" or not beardStyle then
        ...
        local beardColor = normalizeColor(
            characterData[FZ_ENUM_CHARACTER_INFO_BEARD_COLOR],
            hairColor
        )
        if beardColor then
...
function CHARACTER:RestorePosition(x, y, z, angles)
    ...
    if isoPlayer.setX then isoPlayer:setX(x) end
    if isoPlayer.setY then isoPlayer:setY(y) end
    if isoPlayer.setZ then isoPlayer:setZ(z) end
    if isoPlayer.setLx then isoPlayer:setLx(x) end
...
function CHARACTER:RestoreTraits(characterData)
    ...
    -- Clear all existing traits and restore from saved data
    if CharacterTrait then
        -- B42: getTraits() removed; use getCharacterTraits(). Trait names are resolved
        -- via CharacterTrait.get(ResourceLocation.of(name)) — works for both B42 saves
        -- (enum name strings like "BRAVE") and B41 saves (PascalCase strings).
        local charTraits = isoPlayer:getCharacterTraits()
        local toRemove = {}
        local known = charTraits:getKnownTraits()
        for i = 0, known:size() - 1 do table.insert(toRemove, known:get(i)) end
        for _, t in ipairs(toRemove) do charTraits:remove(t) end
        for _, traitName in ipairs(traitsData) do
            local traitType = CharacterTrait.get(ResourceLocation.of(traitName))
            if traitType then charTraits:add(traitType) end
        end
    else
        -- B41 fallback
...
function FrameworkZ.Characters:ProcessCreationData(creationData, player)
    ...
    local rawHairStyle = creationData[FZ_ENUM_CHARACTER_INFO_HAIR_STYLE]
    if rawHairStyle == nil or rawHairStyle == "" or rawHairStyle == "Bald" or rawHairStyle == "None" then
        characterData[FZ_ENUM_CHARACTER_INFO_HAIR_STYLE] = ""
    else
        characterData[FZ_ENUM_CHARACTER_INFO_HAIR_STYLE] = tostring(rawHairStyle)
    end
...
local function ensureCharactersStorage(self)
    if type(self.List) ~= "table" then
        print("[FrameworkZ] Warning: Characters.List was overwritten with type '" .. tostring(type(self.List)) .. "' value '" .. tostring(self.List) .. "'. Reinitializing table.")
        self.List = {}
    end

    if type(self.Cache) ~= "table" then
        print("[FrameworkZ] Warning: Characters.Cache was overwritten with type '" .. tostring(type(self.Cache)) .. "' value '" .. tostring(self.Cache) .. "'. Reinitializing table.")
        self.Cache = {}
    end
end
...
function FrameworkZ.Characters:FindCharacterByName(name)
    ensureCharactersStorage(self)

    for _, character in pairs(self.List) do
