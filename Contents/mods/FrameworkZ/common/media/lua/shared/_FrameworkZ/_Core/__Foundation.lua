...
function FrameworkZ.Foundation:TeleportToLimbo(isoPlayer)
    ...
    if isClient() then
        ...
        if FrameworkZ.Timers:Exists(timerName) then
        ...
        FrameworkZ.Timers:Create(timerName, 1, 0, function()
            ...
            if attempts >= maxAttempts then
                print("[FZ] WARNING: Limbo floor validation timed out after " .. tostring(maxAttempts) .. " attempts.")
                FrameworkZ.Timers:Remove(timerName)
            end
        end)
    end
...
function FrameworkZ.Foundation:ValidateAndCreateLimboFloor(x, y, z, attempt)
    ...
    if (attempt or 1) > 1 then
        print("[FZ] Limbo location grid square valid at (" .. x .. ", " .. y .. ", " .. z .. ") after " .. tostring(attempt) .. " attempts")
    else
...
function FrameworkZ.Foundation:CreateFloorTile(x, y, z)
    ...
    -- In B42 the pathfinder reads chunk data during addFloor; if the chunk isn't loaded
    -- yet the call throws a NullPointerException. Defer and let the timer retry.
    if not gridSquare:getChunk() then
        return false
    end
