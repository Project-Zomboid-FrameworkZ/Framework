...
function FrameworkZ.UI.CharacterView:updateAppearance()
    ...
    if hairStyle and hairStyle ~= "" and hairStyle ~= "Bald" and hairStyle ~= "None" then
    ...
    for _, slot in ipairs(equipmentSlots) do
        -- B42: setWornItem requires ItemBodyLocation object instead of a string
    ...
    if equipment then
        for slot, itemData in pairs(equipment) do
            ...
            if itemType and itemType ~= "" and itemType ~= "None" then
                local item = instanceItem(itemType) -- B42: InventoryItemFactory removed; use instanceItem
                if item then
                    ...
                    -- Equip item first (B42: setWornItem requires ItemBodyLocation object)
                    local resolvedSlot = ItemBodyLocation and ItemBodyLocation.get(ResourceLocation.of(slot)) or slot
