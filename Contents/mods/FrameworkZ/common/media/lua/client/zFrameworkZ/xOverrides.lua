...
function FrameworkZ.Overrides:InstallMainScreenOverrides()
...
function FrameworkZ.Overrides:OnGameStart()
    self:InstallMainScreenOverrides()
    LoadMainScreenPanelInt(true)
end

FrameworkZ.Overrides:InstallMainScreenOverrides()
...
function FrameworkZ.Overrides.FormatCustomFieldDisplayValue(value)
    ...
    elseif valueType == "table" then
        ...
        for k, v in pairs(value) do
            ...
            if type(v) == "table" then
                valueText = FrameworkZ.Overrides.FormatCustomFieldDisplayValue(v)
            else
                valueText = tostring(v)
            end

            if valueText ~= nil and valueText ~= "" then
                table.insert(parts, valueText)
            end
        end
            ...
            elseif type(v) == "table" then
                ...
                if getFunc then
                    ...
                    if type(values) == "table" then
                        layout:setValue(FrameworkZ.Overrides.FormatCustomFieldDisplayValue(values), 1, 1, 1, 1)
                    else
                    ...
                    if type(displayValue) == "table" then
                        layout:setValue(FrameworkZ.Overrides.FormatCustomFieldDisplayValue(displayValue), 1, 1, 1, 1)
                    else
...
-- Vanilla crash guard: ISHotbar.onKeyKeepPressed (media/lua/client/Hotbar/ISHotbar.lua) does
-- `getTimestampMs() - self.keyPressedMS` and throws "__sub not defined for operands" if
-- self.keyPressedMS is nil. ISHotbar:new() never initializes keyPressedMS, and the only place
-- that sets it (onKeyStartPressed) can return early without setting it (e.g. while the hotbar
-- radial menu is visible), so a held key can trigger onKeyKeepPressed before it's ever set.
-- Reassigning here (before vanilla's OnGameStart handler registers the event listener) means
-- vanilla ends up registering this wrapper instead of the raw function, since Events.OnKeyKeepPressed.Add
-- reads whatever ISHotbar.onKeyKeepPressed currently is at the time OnGameStart actually fires.
FrameworkZ.Overrides.ISHotbar_onKeyKeepPressed_old = ISHotbar.onKeyKeepPressed

function FrameworkZ.Overrides.ISHotbar_onKeyKeepPressed(key)
    local hotbar = getPlayerHotbar(0)
    if hotbar and hotbar.keyPressedMS == nil then
        hotbar.keyPressedMS = getTimestampMs()
    end

    return FrameworkZ.Overrides.ISHotbar_onKeyKeepPressed_old(key)
end
ISHotbar.onKeyKeepPressed = FrameworkZ.Overrides.ISHotbar_onKeyKeepPressed
