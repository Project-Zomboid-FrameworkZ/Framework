			...
			elseif string.find(line,'^function') or string.find(line,'^local%s+function') then
				...
				if pos_fn then
					...
					if string.sub(fn,1,1)=='(' then
						...
						local colon = string.find(fn0,'[%:]')
						local dot = string.find(fn0,'[%.%:]')
						local unusedStart, classStart = string.find(fn0,'.*[%.]')
						unusedStart, classEnd = string.find(fn0,'.*[%.%:]')
						if dot or (classStart and classEnd) then -- it's a method
							local klass = classStart and classEnd and string.sub(fn,classStart,classEnd-1) or string.sub(fn,1,dot-1)
							local method = string.sub(fn,dot+1)
							--TCore_IO_writeln('function ' .. klass .. '::' .. method .. ftail .. '{}')
							--TCore_IO_writeln(klass .. '::' .. method .. ftail .. '{}')
							outStream:writeln('/*! \\memberof ' .. klass .. ' */ ' .. (colon and string.gsub(method, ":", "::") or method) .. '{}' )
						else
